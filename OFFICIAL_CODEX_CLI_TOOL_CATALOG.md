...
descriptions and retained-field descriptions. Official Codex keeps their
output schemas as internal Code Mode metadata and omits them from direct
Responses declarations; this package omits them entirely because it has no
Code Mode runtime. Unified child processes receive the official non-color,
UTF-8 locale, and pager environment normalization except `CODEX_CI`, which
this package deliberately does not set.
