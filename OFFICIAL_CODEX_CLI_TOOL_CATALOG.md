...
- **Main-branch check:** commit
  `2b5bdcf67547860f2e5c5a605009a70026796b2b`, inspected August 2, 2026.
...
| Build next                | `web.run`                                                                 | Best remaining Codex-specific capability. It provides search, page navigation, image search, finance, weather, sports, and time through one tool, but requires citation/reference persistence and exact history handling. |
...
**Recommended order:** finish `web.run`; then decide whether structured user
questions are desirable; add `tool_search` only when the optional tool count
justifies deferred discovery.
...
| `web.run`           | Client-executed namespaced tool that sends structured search commands to Codex's search service. It returns model-facing text while retaining structured result metadata separately. | Requires provider web-search capability and namespace support. It is automatic for Responses Lite; otherwise the under-development standalone-web-search feature must be enabled. Disabled web-search mode removes it. | Highest-value missing feature, especially for source links and non-page lookups. Exact parity requires a search backend, reference store, canonical/display text separation, lifecycle events, and namespaced-tool serialization. Pi's public `Tool` type is flat, so exact `web.run` wire shape also needs provider-level work or a deliberate flattened alias. | **Build next.**                                    |
...
2. **Implement `web.run` next**, using the existing reference-store and
   canonical-history design rather than a text-only shortcut.
...
- [Codex repository at the inspected main commit](https://github.com/openai/codex/tree/2b5bdcf67547860f2e5c5a605009a70026796b2b)
