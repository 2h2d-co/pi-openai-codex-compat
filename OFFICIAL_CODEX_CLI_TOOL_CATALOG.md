...
browser: pressing `s` there terminates every background terminal. One-shot
...
The newest GitHub prerelease observed was `0.150.0-alpha.11`; prerelease source
...
| Area                           | Alignment                                                                                                                                                                                                  |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Names and descriptions         | All three tool names, top-level descriptions, and descriptions for every retained input field are copied exactly, including Windows guidance.                                                              |
| Unified output schema          | `exec_command` and `write_stdin` use the exact official closed output schema; `shell_command` has no output schema in either implementation.                                                               |
...
| Extra model guidance              | Uses the official tool descriptions.                                                                                                                                                                                                 | Retains those descriptions and also supplies Pi `promptSnippet` and `promptGuidelines` text describing tool pairing and working-directory use.                                       | Pi prompt integration.         |
| Parser compatibility              | `shell_command` accepts the unadvertised legacy `timeout` alias for `timeout_ms`.                                                                                                                                                    | The closed schema accepts only `timeout_ms`.                                                                                                                                         | Uncopied legacy alias.         |
| Numeric domain                    | Rust inputs use `u64`, `usize`, and `i32`; zero yield values are accepted and then clamped, zero timeout is accepted, and timeout has no schema-level upper bound.                                                                   | Numbers must be safe integers. Yield and timeout values must be positive, session IDs are not restricted to `i32`, and timeout is capped at 2,147,483,647 ms for Node timers.        | Runtime validation difference. |
...
| PowerShell launch           | Login mode omits `-NoProfile`; non-login mode adds `-NoProfile`.                                                                                                                                         | Also adds `-NoLogo`; non-login mode adds `-NonInteractive`. The package additionally has a legacy WSL `bash.exe` stdin-launch path.                                                | Observable shell-argument difference. |
| `apply_patch` in shell text | Detects and intercepts eligible `apply_patch` shell invocations, routing them through Codex's patch runtime.                                                                                             | Does not intercept shell text; the extension's separate `apply_patch` tool remains the supported mutation path.                                                                    | Open parity gap.                      |
...
| Token accounting            | Counts approximate tokens from raw observed bytes before lossy UTF-8 conversion.                                                                                                                         | Counts decoded UTF-8 bytes; invalid byte sequences can therefore produce a different estimate.                                                                                     | Minor open discrepancy.               |
| Output timing               | After a successful non-empty PTY write, sleeps 100 ms before starting the requested poll. Empty-poll maximum is configurable and defaults to 300,000 ms. Elicitation pauses extend collection deadlines. | Starts polling immediately, fixes the empty-poll maximum at 300,000 ms, and has no Codex elicitation-pause clock adjustment.                                                       | Open behavioral gap.                  |
...
| Errors                      | Uses official `write_stdin failed: ...`, unknown-process, closed-stdin, spawn, and write error wording.                                                                                                  | Uses Pi-specific error prefixes and wording.                                                                                                                                       | Textual parity gap.                   |
...
| One-shot cancellation        | Sends `SIGTERM`, allows 50 ms for cleanup, then escalates to process-group kill.                                                                                           | Force-kills the process tree immediately.                                                                                                                | Open behavioral gap.                |
| Registry pruning             | At the 64-process soft cap, protects the eight most recently used sessions, prefers unlocked exited sessions, and avoids pruning a session with an active interaction.     | Prefers any exited session and otherwise removes the oldest session, without the protected-eight policy.                                                 | Open parity gap.                    |
| Background transcript        | Continuously emits output deltas and a terminal completion event, while retaining a capped transcript independent of model polls.                                          | Streams Pi updates only during an active tool interaction; later output remains buffered until `write_stdin`. No independent completion cell is emitted. | Pi lifecycle gap.                   |
| Process-control API          | Core/app-server APIs can list sessions and terminate one session; the Codex TUI exposes `/ps`, `/stop`, and the `/clean` alias, with `/stop` stopping all.                 | The extension manager exposes list and stop-all behavior to `/ps`; it has no user-facing per-session termination action.                                 | Intentional current scope.          |
| `/ps` presentation           | Adds a static history cell containing command snippets and recent output chunks, up to 16 processes.                                                                       | Opens a Pi browser showing session ID, OS PID, PTY/pipe mode, command, and cwd. It does not show recent chunks. Non-TUI mode emits a text notification.  | Pi UI adaptation.                   |
| Stop shortcut                | Uses separate `/stop` or `/clean` commands.                                                                                                                                | Registers no `/stop`; pressing `s` in the `/ps` browser stops every current unified session.                                                             | Explicitly requested adaptation.    |

These differences are cataloged rather than automatically treated as defects.
The approved exclusions and Pi output behavior should remain stable. The open
parity gaps are candidates for separate decisions if exact runtime behavior is
later preferred.
