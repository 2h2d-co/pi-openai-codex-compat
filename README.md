...
| Image tool instructions     | The package retains the server-reserved image-generation JSON Schema but uses a Pi-adapted description, prompt snippet, and system-prompt guidelines instead of Codex Code Mode and `view_image` instructions.                                                                                                                                   |
...
The tool retains Codex's server-reserved JSON Schema while using a Pi-native description, one-line system-prompt snippet, and four high-signal guidelines. Local images are inspected with Pi's `read` tool, and generated image content is displayed and saved automatically without Codex Code Mode wrappers.
