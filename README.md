...
| Image tool instructions     | The package retains the server-reserved image-generation wire schema while replacing Rust-specific path annotations and Codex Code Mode instructions with Pi-native descriptions, a prompt snippet, and system-prompt guidelines. Pi's local schema adds image-count bounds and selector exclusivity before execution.                           |
...
The tool uses Pi-native descriptions, including an absolute-path annotation that names the supported image formats and Pi's filesystem behavior. Pi's local schema limits each edit selector to five images and rejects simultaneous local-path and recent-image selection before execution. OpenAI rejects those additional keywords on the reserved tool declaration, so provider and compaction serialization substitute the accepted wire schema while retaining the same fields and descriptions. Pi strips an optional leading `@` from local paths and lexically normalizes `.` and `..` segments before reading them. A one-line system-prompt snippet and four high-signal guidelines cover normal model use. Local images are inspected with Pi's `read` tool, and generated image content is displayed and saved automatically without Codex Code Mode wrappers.
