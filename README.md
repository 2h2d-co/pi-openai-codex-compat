...
The compatibility baseline is official Codex CLI `0.147.0`, released August 7, 2026, at commit `be6e8eac029b183056b7e4402879f15d2c85f61b`. This release made the previously inspected post-`0.146.0` default-tool namespace contract official: Responses Lite groups direct function and custom declarations into one canonical `functions` namespace for namespace-capable providers. The [official release compatibility log](OFFICIAL_CODEX_CLI_RELEASES.md) is the canonical release-by-release record of protocol and `apply_patch` alignment, intentional deviations, and excluded runtimes. This section is the package's user-facing compatibility contract. See the [Responses Lite compatibility report](RESPONSES_LITE_COMPATIBILITY.md) and [Codex caching and transport comparison](CODEX_CACHE_RESEARCH.md) for detailed request-path findings and live cache trajectories.
